...
test.skip(
  process.env.APP_MODE !== "interactive",
  "Fluxo disponível somente no modo interativo local.",
);
...
test("processa lote txt e navega para sessão", async ({ page }) => {
  ...
  await page.route("**/sessions/test-session-flow*", async (route) => {
    await route.fulfill({
      status: 200,
      contentType: "text/html",
      body: "<main><h1>Sessão sintética de teste</h1></main>",
    });
  });
...
test("expande zip no cliente e envia cada txt interno", async ({ page }) => {
  ...
  await page.route("**/sessions/test-session-zip*", async (route) => {
    await route.fulfill({
      status: 200,
      contentType: "text/html",
      body: "<main><h1>Sessão sintética de teste</h1></main>",
