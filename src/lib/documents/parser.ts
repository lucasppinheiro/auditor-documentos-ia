...
const HASH_PATTERN = /^AUD[0-9A-Za-z-]+$/;
...
export type FieldSource = "parser" | "gemini" | "openai" | "unresolved";
export type FieldSources = Record<ExtractedFieldName, FieldSource>;
...
export function parseUploadedDocument(
  ...
  return {
    ...
    normalized,
    fieldSources: buildParserFieldSources(normalized, warningCodes),
  };
}

export function buildParserFieldSources(
  normalized: ParsedUploadedDocument["normalized"],
  warningCodes: Iterable<ParseWarningCode> = [],
): FieldSources {
  ...
  return Object.fromEntries(
    EXTRACTED_FIELD_NAMES.map((field) => {
      ...
      const invalidByWarning =
        (field === "status" && warnings.has("STATUS_UNRECOGNIZED")) ||
        (field === "verificationHash" && warnings.has("HASH_MALFORMED"));

      return [
        field,
        value === null || value === undefined || value === "" || invalidByWarning
          ? "unresolved"
          : "parser",
      ];
    }),
