...
const parserFieldSources = Object.fromEntries(
  EXTRACTED_FIELD_NAMES.map((field) => [field, "parser"]),
) as FieldSources;
...
const sampleDocuments: SessionDocumentView[] = [
  {
    ...
    serviceDescription: "Serviço sintético de tecnologia",
    supplierName: "Aurora Tecnologia Fictícia",
    supplierCnpj: "10000000000101",
    grossAmount: 8500,
    issueDateIso: "2025-01-29",
    paymentDateIso: "2025-02-07",
    invoiceIssueDateIso: "2025-01-28",
    approvedBy: "Aline Rocha",
    destinationBank: "Banco Exemplo 001",
    status: "PAGO",
    verificationHash: "AUD123",
    extractionMethod: "parser-only",
    modelId: "parser",
    promptVersion: "document-extractor-v2",
    processedAt: "2026-04-17T08:05:00.000Z",
    notExtractedFields: [],
    fieldSources: parserFieldSources,
    anomalies: [
      {
  ...
  },
  {
    ...
    serviceDescription: "Serviço sintético operacional",
    supplierName: "Horizonte Serviços Fictícios",
    supplierCnpj: "20000000000102",
    grossAmount: 1240,
    issueDateIso: "2025-02-10",
    paymentDateIso: "2025-02-12",
    invoiceIssueDateIso: "2025-02-11",
    approvedBy: "Bruno Tavares",
    destinationBank: "Banco Exemplo 002",
    status: "PAGO",
    verificationHash: "AUD456",
...
describe("ResultsExplorer", () => {
  ...
  it("filters by anomaly and updates the details panel when another row is selected", () => {
    ...
    expect(screen.getAllByText(/Horizonte Serviços Fictícios/i).length).toBeGreaterThanOrEqual(2);
  ...
  it("renders duplicate evidence with current record and reference record", () => {
    ...
    expect(screen.getByText(/critério usado: Aurora Tecnologia Fictícia \+ NF-1001/i)).toBeInTheDocument();
  ...
  it("paginates the reviewed table for large document batches", () => {
    ...
    expect(within(reviewTable).getByText("DOC_0026.txt")).toBeInTheDocument();
  ...
  it("shows service description, model id, and fields not extracted in the inspection panel", () => {
    ...
    expect(screen.getByText(/serviço sintético operacional/i)).toBeInTheDocument();
