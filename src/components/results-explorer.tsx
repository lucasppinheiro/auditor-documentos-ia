...
export function ResultsExplorer({
  ...
  const severityTotals = documents.reduce(
    (totals, document) => {
      for (const anomaly of document.anomalies) {
        totals[normalizeSeverity(anomaly.severity)] += 1;
      }
      return totals;
    },
    { high: 0, medium: 0, low: 0 },
  );
  const resolvedExportBasePath =
  ...
  return (
    <div className="stagger-group space-y-4">
        ...
        <div className="mt-5 flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            <Link
              ...
              href={`${resolvedExportBasePath}/results.xlsx`}
            >
            ...
            <Link
              ...
              href={`${resolvedExportBasePath}/audit.xlsx`}
            >
      ...
      <section className="surface-panel rounded-[10px] px-5 py-5" aria-labelledby="executive-summary-title">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            <p id="executive-summary-title" className="section-kicker">Visão executiva</p>
            <p className="mt-2 text-sm text-[var(--text-muted)]">
              Distribuição dos achados, principais regras e qualidade do processamento.
            </p>
          </div>
          <span className="meta-chip">Apoio à revisão humana</span>
        </div>
        ...
        <div className="mt-5 grid gap-4 lg:grid-cols-3">
          <ExecutiveBars
            ...
            items={[
              { label: "Alta", value: severityTotals.high, color: "var(--danger)" },
              { label: "Média", value: severityTotals.medium, color: "var(--warning)" },
              { label: "Baixa", value: severityTotals.low, color: "var(--accent-strong)" },
            ]}
          />
          <ExecutiveBars
            title="Principais tipos"
            items={anomalyBreakdown.slice(0, 3).map((item) => ({
              label: formatAnomalyType(item.type),
              value: item.count,
          ...
          <ExecutiveBars
            ...
            items={[
              { label: "Processados", value: documents.length - partialDocuments, color: "var(--success)" },
              { label: "Parciais", value: partialDocuments, color: "var(--warning)" },
              { label: "Sem divergências", value: cleanDocuments, color: "var(--accent-strong)" },
            ]}
          />
        </div>
      </section>
        ...
        <aside className="surface-panel rounded-[10px] px-4 py-4">
          ...
          {selectedDocument ? (
            <div className="inspection-panel mt-4">
              ...
              <PanelSection title="Proveniência por campo">
                <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-1 2xl:grid-cols-2">
                  {Object.entries(selectedDocument.fieldSources).map(([field, source]) => (
                    <div key={field} className="surface-subtle rounded-[6px] px-3 py-2">
                      <p className="text-xs font-semibold text-[var(--text)]">
                        {formatFieldName(field)}
                      </p>
                      <p className="mt-1 text-xs text-[var(--text-muted)]">
                        {formatFieldSource(source)}
                      </p>
                    </div>
                  ))}
                </div>
...
function ExecutiveBars({
  ...
  return (
    <article className="surface-subtle rounded-[8px] px-4 py-4">
        ...
        {items.map((item) => (
          <div key={item.label}>
            <div className="flex items-center justify-between gap-3 text-xs">
              ...
              <span className="ledger-number text-[var(--text)]">{item.value}</span>
            </div>
            <div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-[var(--surface)]">
              <div
                className="h-full rounded-full"
                style={{ background: item.color, width: `${(item.value / maximum) * 100}%` }}
              />
            </div>
          </div>
        ))}
...
function formatFieldSource(source: string) {
  ...
  if (source === "gemini") return "Preenchido por Gemini";
  if (source === "openai") return "Preenchido por OpenAI";
